...
void DrawGame(void)
{
	...
	for (X = 0; X < 3; X++)
		for (Y = 0; Y < 3; Y++)
			if (EnemyStates[X][Y])
			{
				bufferDrawImage(&fb,90 + (X * 54), 48 + (Y * 47), enemy_data,enemy_width,enemy_height,false,false,false);
			}
	...
	for (X = 0; X < 3; X++)
	{
		if (PlayerStates[X])
		{
			bufferDrawImage(&fb,90 + (X * 54), 180, player_data, player_width, player_height,false,false,false);
		}
	}
}
...
void game(void)
{
    ...
    if ((buttons & BUTTON_Z_MASK) && !(prevbuttons & BUTTON_Z_MASK))
    {
		if (CanMove)
		{
			MoveRight();
	...
	printNumber(lcdfont_data, 24, 4, 8, Score, 6);
	printNumber(lcdfont_data, 17, 4, 8, getHiScore(), 6);
	if (LivesLost >= 1)
	{
		for (X = 0; X < LivesLost; X++)
			printMessage(lcdfont_data, 11 + X, 4, 8, "X");
	}
...
void gameOver(void)
{
	...
	if ((buttons & BUTTON_Z_MASK) && !(prevbuttons & BUTTON_Z_MASK)|| 
		((buttons & BUTTON_C_MASK) && !(prevbuttons & BUTTON_C_MASK)))
	{
...
void gameIntro(void)
{
	...
	{
		printNumber(lcdfont_data, 24, 4, 8, Score, 6);
		printNumber(lcdfont_data, 17, 4, 8, Score, 6);
	}
	if (LivesLost >= 1)
	{
		for (X = 0; X < LivesLost; X++)
