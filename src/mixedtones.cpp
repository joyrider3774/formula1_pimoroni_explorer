...
void setupAudio(int32_t sample_rate, uint8_t pin_piezo, uint8_t pin_speaker_enable) 
{
    ...
    gpio_init(speakerEnablePin);
    gpio_set_dir(speakerEnablePin, GPIO_OUT);
    gpio_put(speakerEnablePin, 1);
    ...
    float clock_div = (float)sys_clock / (150000.0f * 256);  // 150kHz carrier, 8-bit resolution
    ...
    for (int i = 0; i < MAX_CHANNELS; i++) {
        ...
        oscillators[i].phase_increment = 0;
    }
    ...
    audioStartTime = time_us_64() / 1000;
}
...
int8_t playTone(float frequency, uint8_t volume, float duration_sec, float delay_sec) {
    ...
    if (delay_sec > 0) {
        oscillators[channel].start_time_ms = (time_us_64() / 1000) + (uint64_t)(delay_sec * 1000);
